...
@router.post("/{record_id}/predict", response_model=AIAnalysisResultResponse)
async def predict_pneumonia_api(
    record_id: int = Path(..., ge=1),
    current_user: User = Depends(get_current_user),
    db: AsyncSession = Depends(async_get_db),
):
    return await predict_record(db, record_id)


@router.get("/{record_id}/analyses", response_model=list[AIAnalysisResultResponse])
async def list_prediction_results_api(
    record_id: int = Path(..., ge=1),
    current_user: User = Depends(get_current_user),
    db: AsyncSession = Depends(async_get_db),
):
    return await list_record_predictions(db, record_id)
